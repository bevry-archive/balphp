...
{	/**
	...
	{	// Prepare
		if ( $upper === true )		$upper 		= 'ABCDEFGHJKMNPQRSTUVWXYZ';
		if ( $lower === true )		$lower 		= 'abcdefghjkmnpqrstuvwxyz';
		if ( $number === true )		$number 	= '123456789';
		if ( $symbols === true )	$symbols 	= '~!@#$%^&*()_+-={}[]:;,.?';
		// Append
		$seed = $upper.$lower.$number.$symbols.$custom;
		$seed_length = strlen($seed);
		// Generate
		$password = '';
		for ( $i = 0, $n = $length, $z = $seed_length-1; $i < $n; ++$i )
		{	// Append random char to password for length
