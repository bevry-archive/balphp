...
{	/**
	...
	{	/*
		...
		{	// The function exists
			...
			if ( $version__existing !== $version__attempting )
			{	// Funcions are not the exact same version
				if ( $set )
				{	// We want to set the function, but we can't as it is already defined
					// so error
					$error_level =
						($version__existing === NULL)
						?	E_USER_ERROR				// don't know
						:	(floor($version__existing) !== floor($version__attempting))
							?	E_USER_ERROR			// different major
					...
					;
					$error_message =
						'<strong>Conflicting function versions</strong><br />'."\r\n".
