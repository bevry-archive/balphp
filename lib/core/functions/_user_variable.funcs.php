	...
	{	//Created by balupton
		...
		{	//
			if( isset($_COOKIE[$name]) ) {
				$value = 
				from_string(trim(
					$stripslashes ? stripslashes($_COOKIE[$name]) : $_COOKIE[$name]
				));
			}
		}
		...
		{	//
			if( isset($_GET[$name]) ) {
				$value = 
				from_string(trim(
					$stripslashes ? stripslashes($_GET[$name]) : $_GET[$name]
				));
			} elseif( isset($_POST[$name]) ) {
				$value = 
				from_string(trim(
					$stripslashes ? stripslashes($_POST[$name]) : $_POST[$name]
				));
