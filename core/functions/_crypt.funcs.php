...
{	/**
	...
	{
		if ( !$CRYPT_ENCODER ) $CRYPT_ENCODER = CRYPT_ENCODER;
		$result = $data;
		$result = trim($result);
...
{	/**
	...
	{
		if ( !$CRYPT_DECODER ) $CRYPT_DECODER = CRYPT_DECODER;
		$result = $data;
		$result = trim($result);
...
{	/**
	...
	function encrypt ( $data, $CRYPT_ENCODER = null, $CRYPT_DECODER = null, $CRYPT_KEY = null, $CRYPT_CIPHER = null, $CRYPT_MODE = null, $CRYPT_SOURCE = null )
	{
		if ( !$CRYPT_ENCODER )	$CRYPT_ENCODER = CRYPT_ENCODER;
		if ( !$CRYPT_DECODER )	$CRYPT_DECODER = CRYPT_DECODER;
		if ( !$CRYPT_KEY )		$CRYPT_KEY = CRYPT_KEY;
		if ( !$CRYPT_CIPHER )	$CRYPT_CIPHER = CRYPT_CIPHER;
		if ( !$CRYPT_MODE )		$CRYPT_MODE = CRYPT_MODE;
		if ( !$CRYPT_SOURCE )	$CRYPT_SOURCE = CRYPT_SOURCE;
